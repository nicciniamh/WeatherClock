...
class si7021:
    ...
    """
    def __init__(self,bus = I2C_BUS, addr = DEVICE_ADDRESS,newmethod=True):
        """ Constructor: 
                Parameters:
        ...
        """
        self.busdev = '/dev/i2c-{:d}'.format(bus)
        try:
            with open(self.busdev) as f:
                smbus2.smbus2.ioctl(f,smbus2.smbus2.I2C_SLAVE,addr)
        except Exception as e:
            sys.stderr.write('Error sending ioctl to {}: {}\n',busdev,e)
            sys.stderr.flush()
    ...
    def temperature(self,rtype=TTYPE_BOTH, precise=False):
        ...
        if not self.newmethod:
            ...
            while msb == 0:
                self.bus.write_byte(DEVICE_ADDRESS,TEMPERATURE)
                time.sleep(.3)
